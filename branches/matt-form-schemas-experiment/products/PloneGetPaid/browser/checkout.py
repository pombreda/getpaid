...
class BaseCheckoutForm( BaseFormView ):
    ...
    def form_fields(self):
        ...
        if fields:
            return fields
        args = []
        if self.form_schemas_name is None:
            return form.Fields()
        formSchemas = component.getUtility(interfaces.IFormSchemas,
                                           name=self.form_schemas_name)
        for section, interface in formSchemas.interfaces.items():
...
class CheckoutAddress( BaseCheckoutForm ):
    ...
    def save_address(self, data):
        ...
        checkout_address_form_schemas = component.getUtility(
            ...
            name="checkout-address-form-schemas")
        ship_address_info = checkout_address_form_schemas.getBagClass('shipping_address')()
...
class CheckoutReviewAndPay( BaseCheckoutForm ):
    ...
    def setUpWidgets( self, ignore_request=False ):
        ...
        checkout_review_and_pay_form_schemas = component.getUtility(
            interfaces.IFormSchemas,
            name='checkout-review-and-pay-form-schemas')
        # edit widgets for payment info
        self.widgets = form.setUpEditWidgets(
            self.form_fields.select( *schema.getFieldNames(
                checkout_review_and_pay_form_schemas.getInterface('payment'))),
    ...
    @property
    def shipping_address_widgets(self):
        formSchemas = component.getUtility(interfaces.IFormSchemas,
                                           name='checkout-address-form-schemas')
        return self._getWidgetsByInterface(formSchemas.getInterface('shipping_address'))

    @property
    def billing_address_widgets(self):
        formSchemas = component.getUtility(interfaces.IFormSchemas,
                                           name='checkout-address-form-schemas')
        return self._getWidgetsByInterface(formSchemas.getInterface('billing_address'))
