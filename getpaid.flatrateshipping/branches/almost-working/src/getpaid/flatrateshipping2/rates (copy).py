...
from getpaid.flatrateshipping2.interfaces import IFlatRateService
import interfaces
...
class MyShippingRateService ( Persistent, Contained ):
    ...
    cf rates.py ups
    """
    interface.implements(interfaces.IFlatRateService, interfaces.IFlatRateSettings)

    def __init__( self ):
        # initialize defaults from schema
        for name, field in schema.getFields( interfaces.IFlatRateSettings ).items():
    ...
    def getRates( self, order ):
        ...
        print FRSA.settings        
        # <getpaid.core.options.FlatRateShippingSettings object at 0xc95a40c>
        print list(providedBy(FRSA.settings))
        #[<InterfaceClass getpaid.flatrateshipping.interfaces.IFlatRateShippingSettings>, <InterfaceClass 
        # getpaid.core.interfaces.IPersistentOptions>]
