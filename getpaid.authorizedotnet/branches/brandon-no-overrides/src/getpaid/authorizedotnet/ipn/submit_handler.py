...
logger = logging.getLogger("getpaid.authorizedotnet")

class Listener(BrowserView):
    """Listener for Authorize.net IPN notifications - registered as a page view
    """
...
#                                                  (self.portal.absolute_url(), orderid))
        ...
        if orderid in order_manager:
            ...
            if response == '1': # Approved
                order.finance_workflow.fireTransition('charge-charging', responsetext)
                cartutil.destroy(self.context)
                logger.debug('received successful IPN payment notification for order %s' % orderid)
                return self.request.response.redirect('%s/@@getpaid.authorizedotnet.thank-you?orderid=%s' %
                                                      (self.portal.absolute_url(), orderid))
        ...
        order.finance_workflow.fireTransition('decline-charging', responsetext)
        return self.request.response.redirect('%s/@@getpaid.authorizedotnet.error?orderid=%s' %
                                              (self.portal.absolute_url(), orderid))
