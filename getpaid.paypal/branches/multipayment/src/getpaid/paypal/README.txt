...
unambiguous value::
    ...
    >>> 'Paypal Website Payments Standard' in browser.contents
    True
    >>> browser.getControl(name="active-payment-processors:list").value = ['Testing Processor', 'Paypal Website Payments Standard']
    >>> browser.getControl(name="submit").click()
    >>> browser.open("http://nohost/plone/@@paypal_payment_settings_page")
    >>> browser.getControl(name="form.server_url").value = ["Sandbox"]
    >>> browser.getControl(name="form.merchant_id" ).value = '12345'
    >>> browser.getControl(name="form.currency").value = ['EUR']
